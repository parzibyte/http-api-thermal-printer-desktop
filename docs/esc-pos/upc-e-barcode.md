...
[Try it on the Playground](../playground.md?operacion=ImprimirCodigoDeBarrasUpcE)
...
Its name is `ImprimirCodigoDeBarrasUpcE`, so it goes on the `nombre` key. Then put the arguments in order
...
```
{"serial":"","nombreImpresora":"Printers_name","operaciones":[{"nombre":"ImprimirCodigoDeBarrasUpcE","argumentos":["02345673",20,380,0]}]}
```
