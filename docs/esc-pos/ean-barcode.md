...
[Try it on the Playground](../playground.md?operacion=ImprimirCodigoDeBarrasEan)
...
Its name is `ImprimirCodigoDeBarrasEan`, so it goes on the `nombre` key. Then put the arguments in order
